...
% --- Expectations ---
% --------------------
close all; clear M P P0;
P.ang    = pi;

% --- Conectivity matrices ---
Nnodes = 5; 
EE = (diag(ones(Nnodes-1,1),1) + diag(ones(Nnodes-1,1),-1) + diag(ones(Nnodes,1)));
EI = diag(ones(Nnodes,1));
II = EE + EI;
IE = EE + .5*(diag(ones(Nnodes-2,1),2) + diag(ones(Nnodes-2,1),-2));

P.A(1).M = log(.25*[0*II EI; 0*IE 1*EE] + 1e-16);

P.A(2).M = log(.5*[0*II 0*EI; 1*IE 0*EE] + 1e-16);% inhibitory

P.A(3).M = log(0*[0*II 0*IE; EI EE] + 1e-32);    % excitatory
    
P.A(4).M = log(0*[II IE; 0*EI 0*EE] + 1e-32);% inhibitory

P.C(1).M = [log(0*ones(Nnodes,1)+ 1e-16);log(1*ones(Nnodes,1)+ 1e-16)];
P.C(1).M(Nnodes+floor(Nnodes*0)+1) = log(2) + P.C(1).M(Nnodes+floor(Nnodes/2)+1);% excitatory exogenous input

P.C(2).M = log(zeros(2*Nnodes,1)+ 1e-32); % inhibitory exogenous input

P.C(3).M = log(zeros(2*Nnodes,1)+ 1e-32);

P.C(4).M = log(zeros(2*Nnodes,1)+ 1e-32);

% --- CA1 inhibitory interneurons ---
for k = 1:Nnodes
    P.P(k).C   = log(.084);                 % [nF] Cell capacitance
    P.P(k).Vl  =    -62;                    % [mV] Leak reversal portential
    P.P(k).gl  = P.P(1).C -log(.012);       % [nF/s] = [nS] Leak conductance
    P.P(k).Vt  = -55;                       % [mV] Spike threshold voltage
    P.P(k).Vr  = -70;                       % [mV] Reset voltage
    P.P(k).Rt  = log(1/1000);               % [s]  Refractory time
    P.P(k).Vg  = [0,-70,0,-70];             % [mV] Reversal portential of the families of synaptic channels
    P.P(k).T   = log([16,5,16,5]/1000);     % [s] Decay time constants for the synaptic conductances
    P.P(k).D   = log(2000);                 % (1/2*sigma^2) [(mV^2)/ms] Membrane noise term
    P.P(k).d   = log(1/1000);
end
% --- CA1 excitatory pyramidal cells ---
for k = Nnodes+1:2*Nnodes
    P.P(k).C   = log(.150);                 % [nF] Cell capacitance
    P.P(k).Vl  =    -65;                    % [mV] Leak reversal portential
    P.P(k).gl  = P.P(2).C - log(0.030);     % [nF/s] = [nS] Leak conductance
    P.P(k).Vt  = -48;                       % [mV] Spike threshold voltage
    P.P(k).Vr  = -70;                       % [mV] Reset voltage
    P.P(k).Rt  = log(8/1000);               % [s]  Refractory time
    P.P(k).Vg  = [0,-70,0,-70];             % [mV] Reversal portential of the families of synaptic channels
    P.P(k).T   = log([16,5,16,5]/1000);   % [s] Decay time constants for the synaptic conductances
    P.P(k).D   = log(4000);                 % (1/2*sigma^2) [(mV^2)/s] Membrane noise term
    P.P(k).d   = log(1/1000);
end
% -------------------
% --- Covariances ---
...
% tic
% [T,Y] = ode15s(odefun,[0 M.opt.t(M.opt.Kend)],y,opt);
% [T1,Y1] = ode23tb(odefun,[0 M.opt.t(M.opt.Kend)],y,opt);
% [T,Y] = ode45(odefun,[0 M.opt.t(M.opt.Kend)],y,opt);
% toc
...
% plot(T,Y(:,65),'c.')
% hold on
plot(M.opt.t(1:M.opt.Kend),squeeze(M.GV(Nnodes+floor(Nnodes/2)+1,1,1:M.opt.Kend)),'.')
plot(M.opt.t(1:M.opt.Kend),squeeze(M.GV(floor(Nnodes/2)+1,1,1:M.opt.Kend)),'.')
