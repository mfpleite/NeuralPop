...
P.A(1).M = [ -32  log(.25); % excitatory
             -32  log(.001)];
    
P.A(2).M = [log(.001) -32;       % inhibitory
            log(1) -32];
...
P.C(1).M = [log(.001) log(3)]'; % excitatory exogenous input
...
P.P(1).Vg  = [0,-70,0,0];               % [mV] Reversal portential of the families of synaptic channels
P.P(1).T   = log([16,5,.5,100]/1000);   % [s] Decay time constants for the synaptic conductances
P.P(1).D   = log(2000);                 % (1/2*sigma^2) [(mV^2)/ms] Membrane noise term
P.P(1).d   = log(1/1000);

P.P(1).optControl(1).chan      = 3;
P.P(1).optControl(1).condgains = [.5 0 0 0]; % conductance contributions to LFP
P.P(1).optControl(1).LdLgains  = [0 -.000]; % gains for [LFP dLFP/dt] signals
P.P(1).optControl(2).chan      = 4;
P.P(1).optControl(2).condgains = -P.P(1).optControl(1).condgains; % conductance contributions to LFP
P.P(1).optControl(2).LdLgains  = P.P(1).optControl(1).LdLgains; % gains for [LFP sLFP/dt] signals
...
P.P(2).Vg  = [0,-70,0,0];             % [mV] Reversal portential of the families of synaptic channels 
P.P(2).T   = log([16,5,.5,100]/1000);   % [s] Decay time constants for the synaptic conductances
P.P(2).D   = log(4000);                 % (1/2*sigma^2) [(mV^2)/s] Membrane noise term
P.P(2).d   = log(1/1000);

P.P(2).optControl(1).chan      = P.P(1).optControl(1).chan;
P.P(2).optControl(1).condgains = [0 -.1 0 0]; % conductance contributions to LFP
P.P(2).optControl(1).LdLgains  = P.P(1).optControl(1).LdLgains; % gains for [LFP sLFP/dt] signals
P.P(2).optControl(2).chan      = P.P(1).optControl(2).chan;
P.P(2).optControl(2).condgains = -P.P(2).optControl(1).condgains; % conductance contributions to LFP
P.P(2).optControl(2).LdLgains  = P.P(2).optControl(1).LdLgains ; % gains for [LFP sLFP/dt] signals
...
M.opt.Drawplots = 0;
M.opt.dttol          = 1e-1;
% M.opt.LCconvTol = 100*M.opt.dttol;
tic
M = spm_DCM_lifpopsys_LC_int(P,M);
fprintf('LC frequency: %f Hz \n', 1/M.opt.T0)
toc
